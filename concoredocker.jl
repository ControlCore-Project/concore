# concoredocker.jl -- Standalone single-file Concore module (Docker paths)
#
...
#     using .Concore
#
# Uses absolute paths (/in, /out) for Docker container execution.
#
...
function parse_port_file(filename::AbstractString)::Dict{String,Int}
    ...
    content = try
        strip(Base.read(filename, String))
    catch
        return result
    end
...
function load_params!()
    ...
    sparams = try
        strip(Base.read(params_path, String))
    catch
        return
    end
...
function default_maxtime!(default::Real)
    ...
    maxtime = try
        parse(Float64, strip(Base.read(maxtime_path, String)))
    catch
        Float64(default)
    end
    return maxtime
...
function concore_read(
    ...
    try
        if _backend isa MmapBackend
            ...
            ins = _wire_content(Base.read(filepath, String))
        end
    catch
        ins = initstr
    end
...
read(port::Int, name::AbstractString, initstr::AbstractString) =
    concore_read(port, name, initstr)

read(port_identifier::AbstractString, name::AbstractString, initstr::AbstractString) =
    concore_read(port_identifier, name, initstr)

write(port::Int, name::AbstractString, val::AbstractVector{<:Real}, delta::Real = 0) =
    concore_write(port, name, val; delta=delta)

write(port_identifier::AbstractString, name::AbstractString, val::AbstractVector{<:Real}, delta::Real = 0) =
    concore_write(port_identifier, name, val; delta=delta)

write(port::Int, name::AbstractString, val::AbstractString, delta::Int = 0) =
    concore_write(port, name, val; delta=delta)

write(port_identifier::AbstractString, name::AbstractString, val::AbstractString, delta::Int = 0) =
    concore_write(port_identifier, name, val; delta=delta)
...
export concore_read, concore_write, read, write, initval, unchanged
